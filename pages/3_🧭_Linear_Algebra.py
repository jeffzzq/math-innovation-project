...
if module == "Vector":
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        ...
        "Part V: The Determinant (Scaling Space)",
        "Part VI: The Anatomy of the Inverse"
    ])
    ...
    with tab5:  # 或者放在你想要的任何 tab 下
        st.header("Part V: The Determinant (Scaling Space)")
        st.markdown("""
        > *"What does the number $ad - bc$ actually measure in the physical world?"*

        In traditional classes, the determinant is just a calculation. But dynamically, **the Determinant is the scaling factor of reality.** When a matrix morphs the spatial grid, it stretches and squishes areas. The determinant tells us exactly how much a $1 \times 1$ unit of area has grown or shrunk.
        """)
        ...
        with col_ctrl:
            st.markdown("#### The Spatial Warping Engine")
            st.caption(
                "Drag the sliders to redefine the basis vectors. Watch how the fabric of space (the grid) reacts.")

            c1, c2 = st.columns(2)
            with c1:
                st.markdown("**New $\hat{i}$ (Red)**")
                a = st.slider("a (i_x)", -3.0, 3.0, 2.0, 0.5, key='det_a')
                c = st.slider("c (i_y)", -3.0, 3.0, 0.5, 0.5, key='det_c')
            with c2:
                st.markdown("**New $\hat{j}$ (Green)**")
                b = st.slider("b (j_x)", -3.0, 3.0, 1.0, 0.5, key='det_b')
                d = st.slider("d (j_y)", -3.0, 3.0, 2.0, 0.5, key='det_d')

            det = a * d - b * c

            st.markdown("---")
            st.latex(rf"\det(M) = ({a})({d}) - ({b})({c}) = \mathbf{{{det:.2f}}}")

            # 极度严谨的文本解说
            if det > 0:
                st.success(
                    f"**Area Scaled by {det:.2f}x**\n\nThe original $1 \\times 1$ square has been stretched into a parallelogram of area {det:.2f}. The space preserves its original **Right-Handed Orientation** ($\hat{{j}}$ is still counter-clockwise to $\hat{{i}}$).")
            elif det < 0:
                st.error(
                    f"**Area Scaled by {abs(det):.2f}x (SPACE FLIPPED)**\n\nNotice the negative sign! The space has been flipped inside out like a mirror image. The green vector crossed the red vector, changing the space into a **Left-Handed Orientation**.")
            else:
                st.warning(
                    "**Area = 0 (DIMENSIONAL COLLAPSE)**\n\nThe basis vectors are collinear (they fell onto the same line). The 2D plane has been crushed into a 1D line. The area is absolutely zero. **This matrix has no inverse** because you cannot un-crush a line back into a plane.")

        with col_plot:
            fig7 = go.Figure()

            # 1. 绘制绝对基准参考系 (The Unit Square & Original Grid)
            fig7.add_trace(go.Scatter(
                x=[0, 1, 1, 0, 0], y=[0, 0, 1, 1, 0],
                fill='toself', fillcolor='rgba(255, 255, 255, 0.1)',
                line=dict(color='white', width=2, dash='dot'),
                name='Original Unit Area (1x1)', hoverinfo='skip'
            ))
            fig7.add_annotation(x=0.5, y=0.5, text="Area=1", showarrow=False,
                                font=dict(color="rgba(255,255,255,0.6)", size=14))
            ...
            # 3. 严格的颜色逻辑
            if det > 0:
                fill_color = 'rgba(46, 204, 113, 0.3)'  # 半透明绿
                line_color = 'rgba(46, 204, 113, 0.8)'  # 网格线绿
                border_color = 'rgba(46, 204, 113, 1.0)'  # 边框绿
            elif det < 0:
                fill_color = 'rgba(231, 76, 60, 0.3)'  # 半透明红
                line_color = 'rgba(231, 76, 60, 0.8)'  # 网格线红
                border_color = 'rgba(231, 76, 60, 1.0)'  # 边框红
            else:
                fill_color = 'rgba(149, 165, 166, 0.6)'  # 灰色 (坍缩)
            ...
            fig7.add_trace(go.Scatter(
                ...
                name=f'Transformed Area = {abs(det):.2f}'
            ))

            # 5. 【终极绝杀】绘制内部扭曲网格 (Warped Internal Grid)
            # 这段代码会让空间看起来像真正的布料被拉扯
            grid_density = 4  # 将内部切分成 4x4 的网格
            if det != 0:  # 只有没坍缩的时候才画网格
                for i in range(1, grid_density):
                    t = i / grid_density
                    # 画平行于新 i 的线
                    start_pt_i = [t * b, t * d]
                    end_pt_i = [a + t * b, c + t * d]
                    fig7.add_trace(go.Scatter(x=[start_pt_i[0], end_pt_i[0]], y=[start_pt_i[1], end_pt_i[1]],
                                              mode='lines', line=dict(color=line_color, width=1.5, dash='dash'),
                                              showlegend=False, hoverinfo='skip'))
                    # 画平行于新 j 的线
                    start_pt_j = [t * a, t * c]
                    end_pt_j = [b + t * a, d + t * c]
                    fig7.add_trace(go.Scatter(x=[start_pt_j[0], end_pt_j[0]], y=[start_pt_j[1], end_pt_j[1]],
                                              mode='lines', line=dict(color=line_color, width=1.5, dash='dash'),
                                              showlegend=False, hoverinfo='skip'))

            # 6. 绘制新的极坐标基向量
            fig7.add_trace(
                go.Scatter(x=[0, a], y=[0, c], mode='lines+markers+text', line=dict(color='#e74c3c', width=5),
                           text=['', 'New i'], textposition='bottom right', name='New i', textfont=dict(size=16)))
            fig7.add_trace(
                go.Scatter(x=[0, b], y=[0, d], mode='lines+markers+text', line=dict(color='#2ecc71', width=5),
                           text=['', 'New j'], textposition='top left', name='New j', textfont=dict(size=16)))

            # 动态标题
            fig7.update_layout(
                title=f"Space Warping Simulator | det(M) = {det:.2f}",
                xaxis=dict(range=[-5, 5], zeroline=True, zerolinewidth=2, zerolinecolor='rgba(255,255,255,0.5)',
                           gridcolor='rgba(255,255,255,0.1)'),
                yaxis=dict(range=[-5, 5], scaleanchor="x", scaleratio=1, zeroline=True, zerolinewidth=2,
                           zerolinecolor='rgba(255,255,255,0.5)', gridcolor='rgba(255,255,255,0.1)'),
                height=550, margin=dict(l=0, r=0, b=0, t=40), template="plotly_dark"
            )
            st.plotly_chart(fig7, use_container_width=True)

        # ==========================================
        # The Math Roots 哲学总结部分 (严谨升级)
        # ==========================================
        st.markdown("### 🏛️ The Core Philosophy")
        st.markdown("""
        When you memorize $\det(M) = ad - bc$, you are missing the poetry of the universe. 

        1. **The Grid Survives:** Notice the internal dashed lines inside the colored parallelogram? Even though the space is violently stretched, all parallel lines remain parallel, and all grid intersections remain evenly spaced. This is the absolute law of a **Linear Transformation**.
        2. **The Mirror Universe (Negative Determinant):** Cross the red vector ($\hat{i}$) completely over the green vector ($\hat{j}$). The area flashes RED. You haven't just stretched space; you've broken its orientation. If a 2D creature was living in that square, its heart would now be on the right side of its body. 
        3. **The Trap of Zero:** Set the sliders so $\hat{i}$ and $\hat{j}$ land on the exact same line (e.g., $(1, 1)$ and $(2, 2)$). The area becomes $0$. The 2D universe is crushed into a 1D string. This visualizes exactly why dividing by zero is impossible in arithmetic, and why **inverting a matrix with a determinant of 0 is impossible in algebra.**
        """)
    ...
    with tab6:
        ...
        If you are told to "cross out a row and a column, multiply, and add a $+-$ sign", and you feel like it's a pile of nonsense—you are right. It makes no sense until you plug in the numbers and look at the geometry.
        """)
        ...
        # Step 1: 设定矩阵与计算体积
        # ==========================================
        st.subheader("🧱 Step 1: The 3D Box & Its Volume")
        st.markdown(
            "Let's define a specific $3 \\times 3$ matrix $A$. Its three columns are three pillars ($\\vec{v_1}, \\vec{v_2}, \\vec{v_3}$) that hold up a slanted 3D box.")

        col_mat, col_vol = st.columns(2)
        with col_mat:
            st.latex(
                r"A = \begin{bmatrix} \mathbf{1} & 2 & 0 \\ \mathbf{0} & 1 & 2 \\ \mathbf{2} & 0 & 1 \end{bmatrix}")
            st.caption("Notice the first column $\\vec{v_1}$ is **[1, 0, 2]**.")
        with col_vol:
            st.markdown("If we calculate the Determinant (the Volume of this box):")
            st.latex(r"\det(A) = 1(1) - 2(-4) + 0 = \mathbf{9}")
            st.info("💡 **The Goal:** The total 3D Volume is **9**. Keep this number in mind!")

        st.divider()

        # ==========================================
        # Step 2: 破解 Minor 和 Cofactor (理论与数字的完美结合)
        # ==========================================
        st.subheader("🔪 Step 2: Unmasking the Minor and Cofactor")
        st.markdown("""
        Let's calculate the Cofactors for the **First Column** ($\\vec{v_1}$). 

        The rule says: *Always cross out the first column.* Why? Because we want to temporarily ignore $\\vec{v_1}$ and focus entirely on the **2D base** formed by $\\vec{v_2}$ and $\\vec{v_3}$. We are trying to find the **Normal Vector** (the perpendicular needle) pointing out of that base!
        """)

        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown("**1. The X-Coord ($C_{11}$)**")
            st.markdown("*Cross out Row 1 (X). Why? To cast a 2D shadow on the Y-Z wall!*")
            st.latex(r"\begin{vmatrix} 1 & 2 \\ 0 & 1 \end{vmatrix} = 1 - 0 = \mathbf{1}")
            st.success("**Sign is +** \n\n($Y \\times Z$ points to $+X$. Naturally right-handed!) \n\n**Result: 1**")

        with c2:
            st.markdown("**2. The Y-Coord ($C_{21}$)**")
            st.markdown("*Cross out Row 2 (Y). Why? To cast a shadow on the X-Z wall!*")
            st.latex(r"\begin{vmatrix} 2 & 0 \\ 0 & 1 \end{vmatrix} = 2 - 0 = \mathbf{2}")
            st.error(
                "**Sign is - (The Fix!)** \n\n($X \\times Z$ points backwards to $-Y$! We MUST fix this visual inversion with a minus sign). \n\n**Result: -2**")

        with c3:
            st.markdown("**3. The Z-Coord ($C_{31}$)**")
            st.markdown("*Cross out Row 3 (Z). Why? To cast a shadow on the X-Y wall!*")
            st.latex(r"\begin{vmatrix} 2 & 0 \\ 1 & 2 \end{vmatrix} = 4 - 0 = \mathbf{4}")
            st.success("**Sign is +** \n\n($X \\times Y$ points to $+Z$. Naturally right-handed!) \n\n**Result: 4**")

        st.info("""
        🤯 **The Revelation:** The Cofactors for the first column are **[1, -2, 4]**. 
        This is NOT a random string of numbers. **It is the exact Normal Vector that is perfectly perpendicular to $\\vec{v_2}$ and $\\vec{v_3}$!**
        """)

        st.divider()

        # ==========================================
        # Step 3: 转置与终极对撞 (数字对撞高潮)
        # ==========================================
        st.subheader("💥 Step 3: The Adjoint and The Collision")
        st.markdown("""
        To create the **Adjoint Matrix**, we take that Normal Vector `[1, -2, 4]` and **Transpose** it, laying it flat to become the **First Row**:
        """)
        st.latex(
            r"\text{adj}(A) = \begin{bmatrix} \mathbf{1} & \mathbf{-2} & \mathbf{4} \\ \dots & \dots & \dots \\ \dots & \dots & \dots \end{bmatrix}")

        st.markdown("""
        Why lay it flat? Because we turned it into a **Scanner**. Now, let's multiply this scanner by the original three pillars of matrix $A$:
        """)

        col_scan1, col_scan2, col_scan3 = st.columns(3)
        with col_scan1:
            st.markdown("**Scanning $\\vec{v_2}$**")
            st.latex(r"\begin{bmatrix} 1 & -2 & 4 \end{bmatrix} \cdot \begin{bmatrix} 2 \\ 1 \\ 0 \end{bmatrix}")
            st.latex(r"= 2 - 2 + 0 = \mathbf{0}")
        ...
        with col_scan3:
            st.warning("**Scanning $\\vec{v_1}$**")
            st.latex(r"\begin{bmatrix} 1 & -2 & 4 \end{bmatrix} \cdot \begin{bmatrix} 1 \\ 0 \\ 2 \end{bmatrix}")
            st.latex(r"= 1 + 0 + 8 = \mathbf{9}")
            st.caption("Base Area × Height = VOLUME!")

        st.divider()

        st.subheader("🗝️ The Grand Conclusion")
        st.markdown("""
        When you multiply $A \cdot \\text{adj}(A)$, the Normal Vectors completely annihilate the vectors they are perpendicular to (creating the `0`s), and perfectly multiply with the remaining vector to calculate the Volume (creating the `9`s).
        """)
        st.latex(
            r"A \cdot \text{adj}(A) = \begin{bmatrix} \mathbf{9} & 0 & 0 \\ 0 & \mathbf{9} & 0 \\ 0 & 0 & \mathbf{9} \end{bmatrix} = \mathbf{9} \cdot I")

        st.markdown("""
        To get the Inverse ($A^{-1}$), we simply divide everything by **9** (the Determinant) to shrink the universe back to its original size. 

        **This is the glorious geometric truth behind the mechanical algebraic nightmare.**
        """)